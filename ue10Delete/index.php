 	...
 	<script>
		...
		function myFunction(){
			...
			if (confirm("Wollen Sie das Projekt wirlich löschen?")){
				...
				var myAjaxConfigObj={
					url: "http://localhost/medt/ue10/index.php", // Default: The current page (http://127.0.0.1/medt/ue10/index.php)
					...
					success: function(dataFromServer, textStatus, jqXHR){
						...
						if (dataFromServer)
						{
							...
							$("#msgBox").text("Löschen nicht erfolgreich").addClass("bg-danger").show(200).delay(2000).hide(200);
					},
				...
				console.log("Ohhh, no!" + currentProId); // JavaSript
...
<body>
	...
	foreach ($db->query($sql) as $item)
	{
		...
		<a href = \"index.php?delateProject = $item -> id\"<span class=\"glyphicon glyphicon-trash deleteicon\" aria-hidden=\"true\"></span>
		";
